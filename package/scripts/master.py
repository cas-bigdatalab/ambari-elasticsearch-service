...
class Master(Script):
    ...
    def install(self, env):
        ...
        File(format("{elastic_base_dir}/elastic-env.sh"),
             owner=params.elastic_user,
        ...
        cluster_path = service_packagedir + '/scripts/start.sh'
        File(cluster_path,
        content=Template("start.sh.j2")
        )
    ...
    def configure(self, env):
        ...
        cmd=format("cd {service_packagedir}/scripts/;chmod u+x start.sh;sh start.sh")  
        Execute(cmd)       
    ...
    def start(self, env):
        ...
        # Start Elasticsearch
        cmd = format("source /etc/profile; {elastic_base_dir}/bin/elasticsearch -d -p {elastic_pid_file}")
        Execute(cmd, user=params.elastic_user)
